...
# Directory to store custom start text for each bot
START_TEXT_DIR = "start_texts"

# Ensure the directory exists
if not os.path.exists(START_TEXT_DIR):
    os.makedirs(START_TEXT_DIR)

# Load start text for the current bot
def load_start_text(bot_id):
    file_path = os.path.join(START_TEXT_DIR, f"{bot_id}.json")
...
# Save custom start text for the current bot
...
async def set_start_text(client, message):
    ...
    bot_id = client.me.id  # Use the bot's unique ID
    new_text = " ".join(message.command[1:])
    save_start_text(bot_id, new_text)
    await message.reply(f"Start text updated for this bot to:\n\n{new_text}")

# Command to display the start text
...
async def start_command(client, message):
    ...
    if len(message.command) != 2:
        ...
        await message.reply_photo(
            ...
            caption=CLONE_START_TXT.format(message.from_user.mention, me2),
            reply_markup=reply_markup
        )
    else:
        bot_id = client.me.id  # Use the bot's unique ID
        start_text = load_start_text(bot_id)
        user_name = message.from_user.first_name
        bot_name = client.me.first_name
        await message.reply(start_text.format(user_name, bot_name), disable_web_page_preview=True)
